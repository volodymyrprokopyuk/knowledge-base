...
- Owner control :: The owner control mechanisms allow to grant or revoke special
  permissions e.g. minting and burning of tokens, suspension and resume of a
  contract, whitelisting and blacklisting of addresses, recovery of funds to a
  set of designated addresses (multisig)
- Token minting and burning :: The minting mechanism creates and transfers
  tokens to recipients designated by the owner. The burning mechanism removes
  and destroys tokens from owners designated by the owner
- Token caps :: The ability to set predefined limits on the total supply of
  tokens
- Funds recovery :: The funds recovery mechanisms are activated by a designated
  set of address and allow to recover funds and reverse transfers
- Address whitelisting and blacklisting :: The whitelisting mechanism allows to
  restrict critical actions to a designated set of addresses. The blacklisting
  mechanism blocks transfers to specific addresses
...
- Deal balance :: The =vm.deal= function sets the balance of an address
  #+BEGIN_SRC solidity
...
- Assert call :: The =vm.expectCall= function asserts whether a call to an
  address with the exact match or the prefix match data has been made, or not
  made at all (~count = 0~)
  #+BEGIN_SRC solidity
function expectCall(
...
- Emit event :: The =vm.expectEmit= function asserts that a sequence of events
  ...
  (is always matched), the topics 1-3, and the data. Multiple =vm.expectEmit= +
  =emit Ev...= patterns can be specified before the call
  #+BEGIN_SRC solidity
...
- Revert error :: The =vm.expectRevert= function asserts that the next call
  ...
  #+BEGIN_SRC solidity
// Asserts a revert with any calldata
