...
- Library :: A =library= is a set of internal and external functions deployed
  once and reused in the context of calling contracts through the =delegatecall=
  for the external functions and the efficient jumps inside EVM for the internal
  functions. All internal library functions referenced by a calling contract are
...
- Low-level calls :: The low-level functions =call()=, =delegatecall()=, and
  ...
  - Delegate call :: The =delegatecall()= function implements the =library=
    feature, preserves the context of the current calling contract, and only
    uses the code from the target address
...
- Exact contract balance is unreliable :: Reverting from the receive function
  and the payable fallback function cannot prevent a contract from receiving
  ether. The actual contract balance can be higher than the internally accounted
  balance
  #+BEGIN_SRC solidity
contract Receive {
...
- Least privilege :: Grant only the minimum necessary rights to a subject to
  perform an action. Enforce subject authorization with Solidity access control
  modifiers
...
- Economy of mechanism :: Keep it simple. Prefer clarity over performance. Only
  use the blockchain for critical data and operations
- Fail securely :: After a failure a contract must be in a secure state by
  handling errors gracefully
- Blockchain properties
  - =block.timestamp= is imprecise on a blockchain
...
- Checks-effects-interactions :: First perform all checks using the =require()=
  ...
  recorded actions are not performed yet. Finally, perform local actions, call
...
- Isolate transactions :: Avoid combining multiple transactions into a single
  ...
  reach the gas limit of a block and revert the whole transaction. Split a large
  array into multiple blocks processed in isolated transactions
...
- Check low-level raw calls :: Explicitly handle the success bool return value
  ...
  ~addr.staticcall()~, ~addr.delegatecall()~. Calls on contract instances
  ~ctr.function()~ revert automatically
...
- Prefer call over transfer and send :: Prefer the ~addr.call{vaule: 1, gas:
  10}()~ over the =addr.transfer()= and =addr.send()=. The transfer and the send
  functions forward only 2300 gas, which may not be enough if the recipient is a
  contract or the cost of EVM instructions changes. The call function forwards
  all available gas
  #+BEGIN_SRC solidity
// Avoid
