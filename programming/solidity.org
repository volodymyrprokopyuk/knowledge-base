...
- Blockchain properties can be manipulated by miners e.g. =block.hash=,
  =block.timestamp=
...
- Reentrancy :: A called external function performs an unanticipated call back
  into the original calling function initiating multiple half-completed
  executions of the original function leading to a cascade of state changes.
  When a contract sends ether to an unauthorized address a a malicious called
  function calls back to the original contract and reenters the process of
  sending ether until all ether is sent or the gas limit is reached.
...
contract Reentrant {
  ...
  function withdraw() external lock {
    ...
    // UNSAGE record the result of already executed action
    balances[withdrawer] = 0;
  }
}
