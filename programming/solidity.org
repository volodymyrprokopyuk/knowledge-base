...
- Dynamic array :: Each element of the dynamic array ~T[] arr;~ is initialized
  with the default value for the element type =T=. The dynamic array with only
  the =storage= data location automatically grows as elements are added to the
  array. The dynamic array with the =memory= data location is not possible to
  resize. The notation is reversed e.g. ~T[][4]~ is the fixed array of 4 dynamic
  arrays. The access is in the opposite direction to the declaration e.g.
  ~t[0][1]~ is the first element of the second dynamic array
...
- Delete :: The =delete= operator assigns a default value or a new default
  reference value to a variable, an array slot, a mapping slot, or a struct
  field. The delete operator assigns an empty fixed or dynamic array to an array
