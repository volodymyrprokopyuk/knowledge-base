...
- Help
    - Object help `?data.table::melt` = `help(melt, data.table)`
    - Package help `package?data.table`
    - Search help `??"melt"` = `help.search("melt")`
- Configuration
    - `~/.Renviron` key=value, library paths
    - `~/.Rprofile` R code, R options
- Packages at `.libPaths()`
    - System (site) packages in `/usr/lib/R/library` with `yay`
    - User packages in `~/R` with `install.packages(...)`
- Naming
    - Ordinary name `a_var`, `a.var`, `aVar`
    - Non-syntactic name `` `_var`, `a var` ``, `` `%...%` <- function(l, r) {...} ``
    - System name, hidden name `.var`
- Assignment
    - Object assignment `varaible <- object`, `(variable <- object)` with printing
    - Function parameter binding `fun(param = value)`
- Anonymous body `{ ...; ... }`
...
- Minimal R package `devtools::create(path)` -> `R/` + `DESCRIPTION` + `NAMESPACE` +
  `[README.md]`
    - `README.md` why to use, how to use, how to get, where to learn
    - `NEWS.md` interface changes in new release
- Package statges
...
- `DESCRIPTION` (fields) package metadata + package dependencies on other packages
    ...
    - `Imports` (just load) mandatory package deps (makes sure the deps package is
      installed), `Suggests` optional package deps <-
...
- `R CMD check` = `devtools::check()` checks a package metadata, structure, dependencies,
  R code, load / unlaod, data, documentation / vignettes, compiled C/C++/Fortran code,
  tests
- `NAMESPACE` (directives) package imports of objects from other packages + package
  exports <- `devtools::document()`
    ...
    - `NAMESPACE` is usually generated by `roxygen2`
        - `#' @export, @exportClass @exportMethod` (export as little as possible)
        - `#' @importFrom, @importClassesFrom, @importMethodsFrom, @useDynLib` makes
          objects available for use without `::` (prefer `DESCRIPTION.Imports` +
          `package::object`)
    - It is common for packages to be listed in `DESCRIPTION.Imports` (installed +
      loaded), but not in the `NAMESPACE` (not attached, accessible via
      `package::object`) (recommended approach)
    - `package:::object` accesses a non-exported object from a package
- Dataset
    - `data/*.Rdata` automatically exported, lazily loaded binary data created with
      `save(object)` + `DESCRIPTION.LazyData: true` that does not use the `NAMESPACE`
      mechanism <- `devtools::use_data()`
    - `R/sysdata.rda` iternal binary data (not exported, available only for package
      functions) not available to the user <- `devloots::use_data(internal = T)`
    - `inst/extdata/*.*` raw data used to show how to load raw data (when a pakcage is
      installed the `inst/*` content is copied into the top-level package directory)
- Release a package to CRAN `devloots::release()` = `devloots::build()` +
  `devtools::check()` + upload the package to CRAN
