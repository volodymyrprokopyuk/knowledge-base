...
- Generic function :: has type parameters for input arguments and return values.
  A generic function is not a Go function, but a *template* used by a Go
...
func Filter[T any](slc []T, pred func(val T) bool) []T {
...
- Type element :: defines a set of *operators* supported by a type parameter.
  Supported operators are an *intersection* of operators from all type terms.
  *Type terms* match types exactly, prepend =~= a *type approximation* to a type
  term to match derived types e.g. type aliases
...
type Number interface {
  Integer | Float // type terms can be other type elements
}
func Neg[T Number](val T) T {
  return -val // enables operators on type parameters
}
...
- Self-referential constraint :: refers to T inside the interface itself. Must
  be defined (the only possible way) as an anonymous constraint literal in order
  to reference T
...
- Generic struct :: has type parameters for struct fields and methods. Generic
  types are always instantiated for concrete types in a compiled program
...
- Type constraints :: any *interface* can be used as a type constraint. An
  interface either defines a set of *method elements* that a type parameter must
  implement, or a set of *type elements* (can only be used as a type constraint)
  that, in turn, specify a set of allowed operators on type parameters
  ...
  - Comparable ~[T comparable]~ comparable, but unordered types that support
    only equality ==, != e.g. bool, struct of comparable fields. Go operators do
    not call methods, hence there is no way to manually define an interface
    equivalent to a language-provided ~comparable~ that also works with
    user-defined comparable structs
...
- src_fish{ go test -v -run Test/Sub } selective testing
