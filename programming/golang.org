...
- An interface implies inefficiency due to the indirection between a static
  ...
  indirection). An interface consists of
...
- Type element :: defines a set of *operators* supported by a type parameter.
  ...
  *Type terms* match types exactly, prepend =~= a *type approximation* to a type
  term to match derived types e.g. type aliases. *Union of type terms* combines
  built-in types, arrays, slices, maps, channels, structs, functions,
  interfaces, and other type elements

#+BEGIN_SRC go
// inline type element that allows derived types
...
Min(1, 2), Min("b", "a") // 1, a
// type element = union of type terms
