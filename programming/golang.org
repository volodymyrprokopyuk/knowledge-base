...
if err != nil {
  ...
  if errors.As(err, &ce) {
    ...
    #+END_SRC
  - High-level errors vs low-level details ::
    #+begin_src go
type CustomError struct {
  Status int
  Code string
...
errLow := errors.New("low-level details")
errHigh := CustomError{
  Status: 500,
  Code: "high_level_failure",
}
errw := fmt.Errorf("%w: %w", errHigh, errLow)
fmt.Printf("%v\n", errw) // 400 high_level_failure: low-level details
errh, assert := errors.AsType[CustomError](errw)
if assert {
  fmt.Printf("%v\n", errh) // 500 high_level_failure
  fmt.Printf("%d %s\n", errh.Status, errh.Code) // 500 high_level_failure
}
    #+end_src
...
- Error handling strategy ::
  - Request ID propagation :: Assign a request ID at the edge service and
    propagate the request ID through the chain of service calls to serve the
    request for traceability
  - Immediate error capture :: Capture errors as soon as possible
  - Boundary error translation :: Classify errors, add context, and translate
    technical errors to domain errors that the service boundary through error
    wrapping that preserves the root cause
  - Deferred error reporting at the edge :: Defer errors reporting to the edge
    services as late as possible
  - Application vs technical error separation :: Separate the high-level,
    human-readable, application error reporting (with a limited set of errors
    through htmx) from the low-level, structured, technical error reporting
    (with all possible errors through slog)
  - Request ID in application errors :: Include the request ID into application
    errors, so the user can report the issue with an actionable reference
  - Error codes for localization :: Return error code suitable for error
    localization, not error message for a specific language

*** panic/recover

- defer :: The =defer= function is automatically executed on =return= or
  =panic()=
...
fmt.Println("continue") // panic: oh, continue
  #+END_SRC
- os.Exit(1) :: ~os.Exit(1)~ immediately terminates the process without running
  =defer= functions and =recover= handlers. Perform manual cleanup before
  calling ~os.Exit(1)~
...
- Function :: all function arguments including pointers are *call-by-value*
  ...
  - Function parameters order :: Place mandatory structural mechanisms and
    functional dependencies on the left, while optional data parameters and
    variadic configuration options on the right of the function parameter list
...
- Package :: A package is a set of types, functions, and methods defined in
  multiple files under a package directory. A package is the smallest unit of
  code organization and code compilation. A package provides clear API
  boundaries by encapsulating private and exporting public identifiers.
  One-word, all-lowercase package name must match the package directory. Every
  source file in the package directory must have the same package name src_go{
  package pkgname }. Top-level Capitalized identifiers are *exported* from a
  package. Top-level lowercase identifiers are *unexported* and private to a
  package. *Circular dependencies* between packages *are not allowed*. Merge
  dependent packages into a single package, or create a new shared package to
...
- Module :: A module is a collection of packages. A module is a unit of
  versioning identified by a repository path ~scm/user/module~. A module with
  its dependencies is defined in =go.mod=. A module represents either
...
- Stable tests are a *valuable software asset* as they define *what* a program
  is doing, while a more *varying code* defines *how* a program is implemented
...
- src_fish{ go clean -testcache } clean test cache
...
package main_test // Test only public functions
import (
  "testing"
  "os"
  "github.com/volodymyrprokopyuk/main" // Test only public functions
)
...
#+BEGIN_SRC go
func postgres() error {
  // The PGX pool is concurrency safe
  pgp, err := pgxpool.New(ctx, os.Getenv("TRAINER_POSTGRES_URL"))
  if err != nil {
    return err
  }
  defer pgp.Close()
  // * Exec does not return rows
  sql := `
begin;
...
commit;`
  _, err = pgp.Exec(ctx, sql)
  if err != nil {
    return err
  }
  // * A transaction with automatic commit on success and rollback on error
  err = pgx.BeginFunc(ctx, pgp, func(tx pgx.Tx) error {
    rows, _ := tx.Query(
      ctx,
      "insert into util.person(name) values ($1), ($2) returning id;", "c", "d",
    )
  ...
  // * A transaction with manual commit on success and rollback on error
  ...
  rows, _ := tx.Query(
    ctx,
    "insert into util.person(name) values ($1), ($2) returning id;", "e", "f",
  )
  ids, err := pgx.CollectRows(rows, pgx.RowTo[int])
  if err != nil {
    return err
  ...
  fmt.Printf("%v\n", ids) // 5, 6
  // A rollback is safe to call even on a committed transaction
  defer tx.Rollback(ctx)
  // * CollectRows into a struct using reflection
  // Query error is safe to skip, as it is deferred to the first row scan
  rows, _ = pgp.Query(
    ctx, "select per.id, per.name from util.person per where per.id = $1;", 1,
  )
  type person struct {
    ID int `db:"id"`
    Name string `db:"name"`
  ...
  fmt.Printf("%+v\n", persons) // [{ID:1 Name:a}]
  // * CollectRows into a struct manually
  rows, _ = pgp.Query(
    ctx, "select per.id, per.name from util.person per where per.id = $1", 2,
  )
  persons, err = pgx.CollectRows(
    rows, func(row pgx.CollectableRow) (person, error) {
      var per person
      err := row.Scan(&per.ID, &per.Name)
      if err != nil {
        return person{}, err
      }
      return per, nil
  ...
  fmt.Printf("%+v\n", persons) // [{ID:2 Name:b}]
  // * ForEachRow for row-by-row processing
  rows, _ = pgp.Query(
    ctx, "select per.id, per.name from util.person per where per.id = $1;", 3,
  )
  var id int
  var name string
