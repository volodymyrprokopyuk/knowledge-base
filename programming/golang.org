...
- Test a module with coverage
  src_fish{ go test -v -cover -coverprofile=coverage.cov }
- HTML coverage report src_fish{ go tool cover -html=coverage.cov }
- Benchmark with memory allocations src_fish{ go test -v -bench=. -benchmem }
- Testing of unexported vs exported functions
