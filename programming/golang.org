...
- Iterator :: a function that yields one value at a time, instead of computing
  all values at once. The iterator is called by the ~for/range~ loop. The
  iterator signature src_go{ iterator func(yield func(i int, val T) bool) }
#+BEGIN_SRC go
