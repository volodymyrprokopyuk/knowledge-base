...
- *Single-threaded event loop* the sequential execution *run to completion** on
  ...
  #+END_SRC
- *Concurrency* splits two or more compound tasks into atomic steps, schedule
...
- *Generators* a new type of function that *does not run to completion** (as a
  regular function does), but *creates an iterator that controls execution of a
  generator**, *suspends maintaining an iternal state* at every `yield` and
  resumes on each iteration call to `it.next()`. `yield` is right-associative
  like `=` and `?:`
- Generator use cases
